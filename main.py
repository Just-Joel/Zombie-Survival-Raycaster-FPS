...
def main():
    pg.init()
    pg.display.set_caption("Zombie Escape - Joel")
    ...
    level_textures = [[0, 0, 0, 0, 1, 4],  # level 0
                      [0, 0, 0, 1, 0, 3],  # level 1
                      [0, 0, 0, 1, 1, 4],  # level 2
                      [0, 0, 0, 0, 0, 1],  # level 3
                      [0, 0, 0, 1, 1, 0],  # level 4
    ...
    while running:
        ...
        if pause:
            ...
            surf2, pause, options, running, newgame, adjust_res, m_vol, sfx_vol, story = pause_menu(
                surf.copy(), menu, pause, options, click, running, m_vol, sfx_vol, sounds, newgame, font, msg, level,
                ticks, hres, story)
            ...
            timer = timer + er / 2
            frame = new_frame(posx - 0.2 * np.cos(rot), posy - 0.2 * np.sin(rot), rot, frame, sky, floor, hres,
                              halfvres,
            ...
            enemies, player_health, mape = enemies_ai(posx, posy, enemies, maph, size, mape, swordsp, ticks,
                                                      player_health, nenemies, level / 3)
            enemies = sort_sprites(posx - 0.2 * np.cos(rot), posy - 0.2 * np.sin(rot), rot, enemies, maph, size, er / 3)
            if exit2 == 0:
                surf = draw_colonel(surf, colonel, posx - 0.2 * np.cos(rot), posy - 0.2 * np.sin(rot), exitx + 0.5,
                                    exity + 0.5,
            ...
            pg.display.set_caption(
                "Health: " + str(round(player_health, 1)) + " Enemies: " + str(nenemies) + " FPS: " + str(
                    fps) + ' ' + msg)
...
def gen_map(size):
    mapc = np.random.uniform(0, 1, (size, size, 1))
        ...
        if enemies[en][8] > 0 and np.random.uniform(0, 1) < 0.1:  # update only % of the time
            ...
            if state == 0 and dist2p < 6:  # normal
                ...
                if dist2p < 0.8 and ticks - cooldown > 10:  # perform attack, 2s cooldown
                    ...
                    player_health = player_health - np.random.uniform(0.1, 1 + level / 3) / np.sqrt(
                        1 + mape[int(posx)][int(posy)])
                    state = 2
...
def load_sounds():
    sounds = {}
    sounds['step'] = pg.mixer.Sound('Assets/Sounds/playerstep.wav')
    sounds['step2'] = pg.mixer.Sound('Assets/Sounds/enemystep.wav')
    sounds['swoosh'] = pg.mixer.Sound('Assets/Sounds/gun.mp3')
    sounds['swoosh2'] = pg.mixer.Sound('Assets/Sounds/gun2.mp3')
    sounds['hurt'] = pg.mixer.Sound('Assets/Sounds/damage.wav')
    sounds['deadmonster'] = pg.mixer.Sound('Assets/Sounds/deadmonster.wav')
    sounds['hitmonster'] = pg.mixer.Sound('Assets/Sounds/hitmonster.wav')
    sounds['hitmonster2'] = pg.mixer.Sound('Assets/Sounds/hitmonster2.wav')
...
def pause_menu(surf, menu, pause, options, click, running, m_vol, sfx_vol, sounds, newgame, font, msg, level, ticks,
               hres, story):
    adjust_res = 1
    ...
    if options == 0:  # main menu
        ...
        pg.draw.polygon(surf, (50, 200, 50),
                        ((123, 414), (123 + 523 * sfx_vol, 414 - 54 * sfx_vol), (123 + 520 * sfx_vol, 418)))
        pg.draw.polygon(surf, (50, 200, 50),
                        ((123, 566), (123 + 523 * m_vol, 566 - 54 * m_vol), (123 + 520 * m_vol, 570)))
        surf.blit(font.render(str(hres) + " x " + str(int(hres * 0.75)), 1, (255, 255, 255)),
                  (200, 220 + 5 * np.sin(ticks - 1)))
        surf.blit(font.render(str(hres) + " x " + str(int(hres * 0.75)), 1, (255, 100, 50)),
                  (202, 220 + 5 * np.sin(ticks)))
...
def splash_screen(msg, splash, clock, font, screen):
    ...
    while running:
        ...
        p_mouse = pg.mouse.get_pos()
        pg.draw.polygon(surf, (200, 50, 50),
                        ((p_mouse), (p_mouse[0] + 20, p_mouse[1] + 22), (p_mouse[0], p_mouse[1] + 30)))
